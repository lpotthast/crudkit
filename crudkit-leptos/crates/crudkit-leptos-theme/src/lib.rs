...
pub fn generate(path: impl AsRef<Path>) -> Result<()> {
    ...
    let themes_file_path = path.join("crudkit-themes.scss");
    let mut file = std::fs::OpenOptions::new()
        ...
        .context("Could not find crudkit-themes.scss after copying comp-time created SCSS_DIR. This must be a bug.")?;
