...
/// Blanket implementation: any type implementing CreateModelTrait also implements IntoSeaOrmActiveModel.
impl<T, A> IntoSeaOrmActiveModel<A> for T
where
    T: CreateModelTrait<A>,
    A: ActiveModelTrait,
{
    fn into_active_model(self) -> impl Future<Output = A> + Send {
        CreateModelTrait::into_active_model(self)
    }
}
...
/// Blanket implementation: UpdateModel can apply to ActiveModel if ActiveModel implements UpdateActiveModelTrait.
impl<U, A> ApplyToActiveModel<A> for U
where
    U: UpdateModelTrait,
    A: ActiveModelTrait + UpdateActiveModelTrait<U>,
{
    fn apply_to(self, active_model: &mut A) {
...
/// Generated by the `CkCreateModel` derive macro.
pub trait CreateModelTrait<A: ActiveModelTrait>: Clone + Send + Sync {
    fn into_active_model(self) -> impl Future<Output = A> + Send;
}

/// Marker trait for UpdateModel.
/// Generated by the `CkUpdateModel` derive macro.
pub trait UpdateModelTrait {}

/// Trait for applying UpdateModel to ActiveModel.
/// Generated by the `CkUpdateModel` derive macro.
pub trait UpdateActiveModelTrait<U: UpdateModelTrait> {
